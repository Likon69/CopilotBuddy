...
| Component | Repo | Role |
| --- | --- | --- |
| **Navigation** (C++ Detour runtime) | [Navigation-C-](https://github.com/Likon69/Navigation-C-) | C++ wrapper around Recast/Detour. Ships `Navigation.dll` (4x4, Trinity / MMAP v5). Based on Honorbuddy's `Tripper.RecastManager` from the WoD / Legion client. |
| **Extractor** — C# | [extractor-csharp](https://github.com/Likon69/extractor-csharp) | Navmesh extractor written in C# / WPF, ported and heavily extended from the MaNGOS extractor. Produces 4x4 sub-tile `.mmtile` files in the Honorbuddy format (PAMM, `mmapVer = 5`). This is the extractor used for the shipped mmaps and the one recommended for generating your own. |
...
The mmap format is 4x4 (Trinity, MMAP v5), the Honorbuddy standard: one ADT = 16 Detour sub-tiles of 133 yards. Generate the tiles with the [C# extractor](https://github.com/Likon69/extractor-csharp).
